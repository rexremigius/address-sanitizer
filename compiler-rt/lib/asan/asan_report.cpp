...
class ScopedInErrorReport {
 public:
  ...
  ~ScopedInErrorReport() {
    // We have modified the condition here to report all the errors that address-sanitizer finds
  ...
  void ReportError(const ErrorDescription &description) {
    ...
    if (current_error_.kind == kErrorKindInvalid) {
      CHECK_EQ(current_error_.kind, kErrorKindInvalid);
    } else if (current_error_.kind != kErrorKindInvalid) {
      CHECK_NE(current_error_.kind, kErrorKindInvalid);
    }
