...
clang -S  -g -emit-llvm -Xclang -disable-O0-optnone -fsanitize=address -fno-omit-frame-pointer -fsanitize-address-use-after-return=never ~/address-sanitizer/Testing/LIT-Test/test_2.c -o ~/address-sanitizer/Testing/test_2.ll

# Using opt to invoke the transformation pass - AddressSanitizer and write the output to out.ll

opt -load-pass-plugin ~/address-sanitizer/build/lib/libAddressSanitizer.so -passes="addressSan" -S ~/address-sanitizer/Testing/test_2.ll -o ~/address-sanitizer/Testing/out_2.ll

# llc is used to convert .ll file to .o file

llc -filetype=obj ~/address-sanitizer/Testing/out_2.ll -o ~/address-sanitizer/Testing/out.o
